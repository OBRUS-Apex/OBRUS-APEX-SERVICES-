...
import { ArrowRight, Award, Zap, CheckCircle, Menu, X, Phone, Mail, Star, LogOut, User } from 'lucide-react';
...
const SERVICES = [
  { t: 'Recruitment & Manpower', d: 'Connecting businesses with skilled professionals across all industrial sectors. Reliable manpower solutions tailored to your operational needs.', i: '👥', border: 'border-[#c8921e]', href: '/recruitment' },
  { t: 'Environmental Services', d: 'Fumigation, waste management, septic tank dislodgement, and janitorial services — fully certified and compliant.', i: '🌿', border: 'border-[#1a7a4a]', href: '/environmental' },
  { t: 'Equipment Procurement', d: 'Sourcing and supplying PPE, safety equipment, and facility materials from verified manufacturers and vendors.', i: '⚙️', border: 'border-[#0b1f3a]', href: '/equipment' },
  { t: 'HSE Consultancy', d: 'Expert Health, Safety, and Environmental advisory — compliance audits, risk assessments, NEBOSH-accredited training.', i: '🛡️', border: 'border-[#c8921e]', href: '/hse' },
  { t: 'Facility Maintenance', d: 'Comprehensive maintenance solutions keeping buildings, equipment, and operational environments running safely.', i: '🏗️', border: 'border-[#1a7a4a]', href: '/environmental' },
  { t: 'General Consulting', d: 'Strategic consulting services supporting businesses in improving operations and solving complex challenges.', i: '💡', border: 'border-[#0b1f3a]', href: '/hse' },
];
    ...
    <img src="/logo.png" alt="OBRUS Apex Services" className="h-8 w-auto" />
          ...
          <div className="hidden md:flex items-center gap-1">
            {NAV_LINKS.map(({ href, label }) => (
              <Link key={href} href={href} className="px-3 py-1.5 rounded-lg text-sm text-white/60 hover:text-[#e8b84b] hover:bg-[rgba(200,146,30,0.1)] transition-all">{label}</Link>
            ))}
            ...
            {user ? (
              ...
              <div className="flex items-center gap-2">
                <Link href="/auth" className="px-4 py-1.5 border border-white/20 rounded-lg text-sm text-white/70 hover:text-white hover:border-white/50 transition-all">Sign In</Link>
                <Link href="/auth" className="px-4 py-1.5 bg-[#c8921e] rounded-lg text-sm font-bold text-[#0b1f3a] hover:bg-[#e8b84b] transition-all">Get Started</Link>
              </div>
            )}
          </div>
          ...
          <div className="md:hidden bg-[#060f1e] border-t border-[rgba(200,146,30,0.1)] px-5 pb-5 pt-3 flex flex-col gap-1" style={{ width: '100%' }}>
            ...
            {user ? (
              ...
              <div className="flex gap-2 mt-1">
                <Link href="/auth" onClick={() => setNavOpen(false)} className="flex-1 px-4 py-3 border border-white/20 rounded-xl text-sm text-white/70 text-center hover:text-white transition-all">Sign In</Link>
                <Link href="/auth" onClick={() => setNavOpen(false)} className="flex-1 px-4 py-3 bg-[#c8921e] rounded-xl text-sm font-bold text-[#0b1f3a] text-center hover:bg-[#e8b84b] transition-all">Get Started</Link>
              </div>
            )}
          </div>
        )}
      ...
      <section className="min-h-screen flex items-center px-5 py-24 bg-[#060f1e] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full">
          ...
          <div className="flex flex-wrap gap-4">
            <Link href={user ? getPortalLink() : '/auth'} className="bg-[#c8921e] hover:bg-[#e8b84b] text-[#060f1e] px-8 py-4 rounded-2xl font-bold text-sm transition-all shadow-xl flex items-center gap-2">
              {user ? 'Go to My Portal' : 'Access Portal'} <ArrowRight size={16}/>
            </Link>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-16 pt-16 border-t border-white/5">
            {[{ n: '400+', l: 'Professionals Placed' }, { n: '150+', l: 'Corporate Clients' }, { n: '6', l: 'Service Divisions' }, { n: '100%', l: 'Safety Record' }].map((stat) => (
      ...
      <section className="py-20 md:py-32 px-5">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {SERVICES.map((svc) => (
              <Link key={svc.t} href={svc.href} className={`bg-white p-7 md:p-10 rounded-2xl border-t-4 ${svc.border} shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group block`}>
                ...
                <div className="flex items-center gap-2 mt-5 text-xs font-bold text-[#c8921e] opacity-0 group-hover:opacity-100 transition-all">Learn more <ArrowRight size={13}/></div>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#060f1e] py-20 md:py-32 px-5">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-14 items-center">
          <div>
            ...
            <h2 className="font-serif text-3xl md:text-5xl text-white font-bold leading-tight mb-6">Redefining Operational Excellence in West Africa.</h2>
            <p className="text-white/40 text-base leading-relaxed mb-10">Headquartered in Port Harcourt, we provide a unified approach to complex corporate challenges — integrating safety, talent, and facility management.</p>
            <div className="space-y-6">
              {[
                ...
                <div key={pt.t} className="flex gap-4">
                  ...
                  <div><h4 className="font-bold text-white mb-1">{pt.t}</h4><p className="text-white/30 text-sm">{pt.d}</p></div>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-[#0b1f3a] rounded-3xl p-10 md:p-14 border border-white/5 shadow-2xl">
            ...
            <p className="text-white/30 leading-relaxed mb-8">"We provide not just staff and supplies, but the peace of mind required to focus on your core growth."</p>
            <div className="space-y-3 mb-10">
              {['HSE Training & Certification', 'Vetted Candidate Matching', 'Environmental Compliance', 'Equipment Supply Chain'].map(item => (
                <div key={item} className="flex items-center gap-3 text-sm text-white/50">
                  <CheckCircle size={14} className="text-[#c8921e] shrink-0"/>{item}
                </div>
              ))}
            </div>
            <Link href={user ? getPortalLink() : '/auth'} className="w-full block text-center py-4 bg-[#c8921e] text-[#0b1f3a] rounded-2xl font-bold text-sm hover:bg-[#e8b84b] transition-all">
              {user ? 'Go to My Portal →' : 'Get Started Today →'}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-32 px-5 bg-[#f5f0e8]">
        <div className="max-w-5xl mx-auto bg-gradient-to-br from-[#060f1e] to-[#0b1f3a] rounded-3xl p-10 md:p-20 text-center relative overflow-hidden shadow-2xl">
          ...
          <h2 className="font-serif text-3xl md:text-5xl text-white font-bold italic mb-6 relative z-10">Start Your Partnership.</h2>
          <p className="text-white/40 text-base md:text-lg max-w-xl mx-auto mb-10 relative z-10">Join dozens of industrial leaders who trust OBRUS for manpower, compliance, and environmental services.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 relative z-10">
            <Link href={user ? getPortalLink() : '/auth'} className="bg-[#c8921e] text-[#060f1e] px-10 py-4 rounded-2xl font-bold hover:bg-[#e8b84b] transition-all">
              {user ? 'Go to My Portal' : 'Access Client Portal'}
            </Link>
            <Link href="/contact" className="border border-white/20 text-white px-10 py-4 rounded-2xl font-bold hover:bg-white/5 transition-all">Talk to a Specialist</Link>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div><span className="block text-white font-bold text-sm">OBRUS</span><span className="text-[#e8b84b] text-[9px] uppercase tracking-widest opacity-60">Apex Services</span></div>
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[['Client Portal', '/client/portal'], ['Employer Portal', '/portal/employer'], ['Sign In', '/auth'], ['Job Listings', '/recruitment']].map(([label, href]) => (
                  <Link key={href} href={href} className="block text-white/30 text-sm hover:text-[#c8921e] transition-all">{label}</Link>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2 text-white/30 text-sm"><Phone size={13} className="text-[#c8921e] mt-0.5 shrink-0"/><span>Port Harcourt, Rivers State</span></div>
                <div className="flex items-start gap-2 text-white/30 text-sm"><Mail size={13} className="text-[#c8921e] mt-0.5 shrink-0"/><span>info@obrusapex.com</span></div>
                <Link href="/contact" className="flex items-center gap-2 text-[#c8921e] text-sm hover:text-[#e8b84b] transition-all"><ArrowRight size={13}/>Contact Us</Link>
              </div>
            </div>
          </div>
