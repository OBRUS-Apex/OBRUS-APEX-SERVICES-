    ...
    <div className="flex min-h-screen bg-[#f5f0e8] text-navy font-sans overflow-hidden">
      ...
      <aside className="w-[280px] bg-[#060f1e] fixed inset-y-0 left-0 border-r border-gold/10 z-[100] flex flex-col no-print">
        ...
        <nav className="p-6 flex-1 space-y-1.5 mt-8">
           <SidebarLink label="Overview" ico={<Home size={18}/>} active={activeTab === 'overview'} onClick={() => setActiveTab('overview')}/>
           <SidebarLink label="Industrial services" ico={<Plus size={18}/>} active={activeTab === 'engage'} onClick={() => setActiveTab('engage')}/>
           <SidebarLink label="Financial Registry" ico={<CreditCard size={18}/>} active={activeTab === 'billing'} onClick={() => setActiveTab('billing')}/>
           <SidebarLink label="Entry tracking" ico={<ClipboardList size={18}/>} active={activeTab === 'activity'} onClick={() => setActiveTab('activity')}/>
        </nav>
      ...
      <main className="ml-[280px] flex-1 flex flex-col h-screen overflow-y-auto custom-scrollbar">
        <header className="h-[80px] bg-white border-b flex items-center justify-between px-12 sticky top-0 z-[50] no-print">
           <h2 className="font-serif text-3xl font-black uppercase tracking-tighter italic decoration-gold underline-offset-[10px] underline decoration-4">
              {activeTab} console
           </h2>
           <div className="flex gap-6 items-center">
              <div className="bg-[#fcfbf9] px-6 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-widest text-slate-400 border border-navy/5 shadow-inner leading-none flex items-center gap-3">
                 <Calendar size={13}/> {new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric'})}
              </div>
              <div className="w-10 h-10 rounded-2xl bg-navy flex items-center justify-center text-white cursor-pointer hover:bg-gold transition-all shadow-xl shadow-navy/20 relative">
                <Bell size={20}/><div className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></div>
              </div>
           </div>
        </header>
        ...
        <div className="p-12 pb-32 w-full max-w-[1400px] mx-auto">
          ...
          {activeTab === 'overview' && (
             <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-14">
                ...
                <div className="bg-[#0b1f3a] p-20 rounded-[60px] text-white relative overflow-hidden shadow-3xl">
                   ...
                   <div className="relative z-10">
                      <h3 className="font-serif text-5xl font-black mb-8 italic tracking-tighter uppercase decoration-gold underline-offset-8 underline decoration-4">Authorization Confirmed</h3>
                      <p className="text-white/40 text-2xl leading-relaxed max-w-2xl font-light">Your portal environment is active and connected to OBRUS Regional Node. You have <b className="text-gold font-bold">industrial clearance</b> to book HSE Training, Waste Management, and Procurement services directly.</p>
                      <button onClick={() => setActiveTab('engage')} className="mt-14 bg-gradient-to-r from-gold to-gold-lt text-navy px-12 py-5 rounded-full font-black text-xs uppercase tracking-[0.4em] shadow-2xl hover:scale-105 transition-all">Establish New Request</button>
                   </div>
                </div>
             </motion.div>
          )}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid md:grid-cols-3 gap-8">
               {[
                 ...
                 { t: 'Custom Operations', d: 'General consultancy & strategy planning.', i: '💡', b: 'border-gold' }
                  ...
                  <div key={svc.t} onClick={() => setSelectedService(svc.t)} className={`bg-white p-12 rounded-[55px] border-t-8 ${svc.b} shadow-sm transition-all hover:shadow-2xl hover:-translate-y-2 cursor-pointer group`}>
                    <div className="text-6xl mb-10 transition-transform duration-700 group-hover:rotate-12">{svc.i}</div>
                    <h3 className="font-serif text-2xl font-black uppercase tracking-tighter mb-4 text-navy group-hover:text-gold transition-colors">{svc.t}</h3>
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest leading-loose">{svc.d}</p>
                  </div>
               ))}
            </motion.div>
          )}
          ...
          {activeTab === 'billing' && (
            <div className="bg-white rounded-[50px] shadow-3xl overflow-hidden border border-navy/5 animate-in slide-in-from-left-6 duration-700">
               <div className="p-10 bg-[#fcfbf9] border-b border-navy/5 flex justify-between items-center">
                  <h3 className="font-serif text-3xl font-black uppercase italic tracking-tighter">Finance ledger</h3>
                  <CreditCard className="text-gold/20" size={38}/>
               </div>
               <table className="w-full text-left">
                  <thead className="bg-[#0b1f3a] text-gold-lt uppercase">
                    <tr>
                      <th className="p-8 text-[11px] font-black tracking-widest">Doc Reference</th>
                      <th className="p-8 text-[11px] font-black tracking-widest">Mission Context</th>
                      <th className="p-8 text-[11px] font-black tracking-widest">Amount Due</th>
                      <th className="p-8 text-[11px] font-black tracking-widest">Protocol Action</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#f0ede6]">
                    {invoices.length === 0 ? (
                      <tr><td colSpan={4} className="p-20 text-center text-slate-400 italic font-medium uppercase tracking-widest text-xs">Zero outstanding balances detected.</td></tr>
                    ) : (
                      invoices.map((inv:any) => (
                        <tr key={inv._id} className="hover:bg-gold/[0.04] transition-all">
                          <td className="p-8 font-mono text-sm font-black text-navy">{inv.id || 'INV-Node'}</td>
                          <td className="p-8 font-serif font-black text-xl text-navy italic">{inv.service}</td>
                          <td className="p-8 font-black text-lg tracking-tighter">₦{inv.amount.toLocaleString()}</td>
                          <td className="p-8">
                             {inv.status === 'pending' ? (
                               <button onClick={() => setSelectedInvoice(inv)} className="bg-navy text-gold-lt px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl border border-gold/20 hover:bg-gold hover:text-navy transition-all transform active:scale-95">Verify & Audit</button>
                             ) : (
          ...
          {activeTab === 'activity' && (
             <div className="bg-white rounded-[55px] shadow-3xl overflow-hidden border border-navy/5 animate-in slide-in-from-right-6 duration-700">
                <table className="w-full text-left">
                  <thead className="bg-[#0b1f3a] text-gold-lt uppercase italic">
                    <tr>
                      <th className="p-9 text-[11px] font-black tracking-[0.3em]">Log Identifer</th>
                      <th className="p-9 text-[11px] font-black tracking-[0.3em]">Division Area</th>
                      <th className="p-9 text-[11px] font-black tracking-[0.3em]">Temporal Record</th>
                      <th className="p-9 text-[11px] font-black tracking-[0.3em]">Network Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#f0ede6]">
                    {requests.length === 0 ? (
                      <tr><td colSpan={4} className="p-32 text-center text-slate-400 font-bold uppercase tracking-widest">Listening for service broadcast nodes...</td></tr>
                    ) : (
                      requests.map((r: any) => (
                        <tr key={r._id} className="hover:bg-gold/[0.04] transition-colors">
                          <td className="p-9 font-mono font-black text-navy text-xs opacity-50 uppercase tracking-widest">{r._id.substring(0,10)}</td>
                          <td className="p-9 font-serif font-black text-2xl text-navy italic tracking-tighter underline decoration-gold/10 decoration-8 underline-offset-[-2px]">{r.serviceType}</td>
                          <td className="p-9 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">{new Date(r.createdAt).toDateString()}</td>
                          <td className="p-9">
      ...
      <AnimatePresence>
        {selectedService && (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-navy-deep/95 backdrop-blur-xl no-print">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-2xl rounded-[60px] p-16 shadow-2xl relative overflow-y-auto max-h-[95vh] custom-scrollbar">
              <button onClick={() => setSelectedService(null)} className="absolute top-12 right-12 bg-navy/5 p-4 rounded-full hover:bg-gold transition-all duration-500"><X size={24}/></button>
              <h3 className="font-serif text-5xl font-black italic tracking-tighter uppercase mb-2">mission dispatch</h3>
              <p className="text-gold text-[10px] font-black uppercase tracking-[0.5em] mb-14 border-l-4 border-gold pl-6">{selectedService} Node Engagement</p>
                 ...
                 <div id="invoice-printable" className="flex-1 p-20 overflow-y-auto">
                    <div className="flex justify-between items-start mb-20">
                       <div className="flex items-center gap-5">
                          <div className="w-16 h-16 bg-navy text-gold rounded-3xl flex items-center justify-center font-black text-3xl uppercase italic">O</div>
                          <div>
                             <h4 className="font-serif text-4xl font-bold tracking-tighter text-navy uppercase leading-none">OBRUS APEX</h4>
                 ...
                 <div className="w-full md:w-[380px] bg-[#f0ede6] p-16 flex flex-col no-print border-l border-navy/5">
                    ...
                    <form onSubmit={handleReceiptUpload} className="space-y-10 flex-1">
                       <div className="relative group">
                          ...
                          <div className="w-full py-16 bg-white border-2 border-dashed rounded-[45px] border-gold/10 flex flex-col items-center justify-center text-center transition-all group-hover:border-gold shadow-sm group-hover:shadow-2xl transform group-hover:-translate-y-1">
                             <Upload size={32} className="text-gold opacity-10 mb-5 group-hover:opacity-100 transition-opacity duration-700"/>
                       ...
                       <button type="submit" className="w-full py-5 bg-navy text-white rounded-[25px] font-black text-xs uppercase tracking-[0.4em] shadow-3xl hover:bg-gold transition-all active:scale-95">Vett Reconciliation →</button>
                    </form>
                    <div className="pt-12 flex flex-col gap-4 mt-auto">
                       <button onClick={handlePrint} className="w-full py-4 bg-white text-navy rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-4 hover:bg-[#0b1f3a] hover:text-white transition-all shadow-xl"><Printer size={16}/> Print Hardcopy</button>
                       <button className="w-full py-4 bg-gold text-navy rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-4 shadow-3xl transform hover:-translate-y-1 transition-all"><Download size={16}/> Archive Digital PDF</button>
                    </div>
                 </div>
              </motion.div>
           </div>
...
function KpiCard({ label, val, unit }: any) {
  return (
    <div className="p-10 bg-white rounded-[50px] border shadow-2xl border-navy/5 relative overflow-hidden transition-all hover:scale-105 hover:border-gold">
       <div className="flex justify-between items-start mb-1 leading-none">
          <h4 className="text-5xl font-serif font-black text-navy italic">{val}</h4>
